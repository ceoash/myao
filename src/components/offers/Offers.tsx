...
const Offers = ({ sent, received, session, multipage }: OffersProps) => {
  ...
  useEffect(() => {
    ...
    socketRef.current &&
      socketRef.current.on("new_listing", (newListing: any) => {
        console.log("new listing", newListing);
        toast.success("New listing added!");
        if (session.user.id !== newListing.userId) {
          setReceivedListings((prevListings) => {
            return {
              ...prevListings,
              [newListing.status]: newListing,
            };
          });
        } else {
          setSentListings((prevListings) => {
