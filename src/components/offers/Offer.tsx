...
const Offer: React.FC<any> = ({
 ...
 const socket = useSocketContext();
  ...
  const handleStatusChange = async (status: string, userId: string) => {
    ...
    confirmation.onOpen(`Are you sure you want to ${formattedStatus} this offer?`, async () => {
      await axios
  ...
  const custom = {
    bids: bids,
    buyer: {id: buyerId, username: buyer.username, profile: {
    ...
    || null}, condition: options?.condition || "", pickup: options?.pickup, public: false},
    price: price, 
    reviews: [],
    seller: {id: sellerId, username: seller.usernamw, profile: {
  ...
  return (
    <div className="w-full border bg-white  border-gray-200 rounded-lg py-1  mb-6  hover:bg-gray-50 transition-all ease-in-out duration-200">
      
      <div className="md:py-4 md:flex md:gap-4 xl:gap-6">
          <Link href={`/dashboard/offers/${id}`} className="w-1/5 overflow-clip rounded-lg md:ml-4">
            <div className="
              relative
              w-full
              h-full
        ...
        <div className="w-full pt-2 pb-2 md:p-0 flex flex-col">
          ...
          <div className="flex flex-grow my-2 px-4 justify-between  items-end text-xs">
            <div className="flex justify-between items-end md:gap-6">
              ...
              >
                <div className=" gap-2 items-center text-xs xl:text-sm hidden md:flex">
                  <FaUser />
                  <div>
              ...
              <div className=" items-center gap-2 text-xs xl:text-sm hidden 2xl:flex">
                <ImPriceTag />
                <span>{type === "sellerOffer" ? "Sale" : "Buy"}</span>
              </div>
              <div className=" items-center gap-2 text-xs xl:text-sm  ">
            ...
            <div className="leading-relaxed lg:p-0 flex gap-2 text-sm ml-auto">
              ...
              { session?.user?.id === userId &&
                ...
                statusState !== "cancelled" && (
                  <>
                    <button
                      onClick={() => edit.onOpen(session?.user?.id, custom, "default", {} )}
                      className={`
                        w-full
                        focus:ring-4 
                        focus:outline-none 
