...
const PreviewPage = ({listing, session, handleStatusChange}: PreviewPageProps) => {
    ...
    if (query.get('success')) {
      isMounted && toast.success('Payment complete -- You will receive an email confirmation shortly with further instructions.');
  ...
  {/*       <input type="hidden" value={listing.buyer.id} name='buyerId' />
      ...
      <section>
      <Button submit options={{ size: "lg" }} accept><FaCreditCard className='mr-1' />Pay</Button>
      </section>
    </form>
    );
